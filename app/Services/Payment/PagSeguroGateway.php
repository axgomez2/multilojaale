...
class PagSeguroGateway implements PaymentGatewayInterface
{
    ...
    public function processPayment(array $data): array
    {
        ...
        try {
            ...
            // Implementação da integração com a API do PagSeguro
            // Este é um exemplo simplificado para fins de demonstração
            
            // Retorno simulado para teste
            return [
                'success' => true,
                'transaction_id' => 'ps_' . uniqid(),
                'message' => 'Pagamento processado com sucesso',
                'status' => 'approved'
            ];
            
        } catch (\Exception $e) {
            Log::error('Erro ao processar pagamento no PagSeguro: ' . $e->getMessage());
    ...
    {
        try {
            // Implementação para consulta de status no PagSeguro
            // Este é um exemplo simplificado para fins de demonstração
            
            return 'approved';
        } catch (\Exception $e) {
            Log::error('Erro ao buscar status do pagamento no PagSeguro: ' . $e->getMessage());
            return 'error';
        }
    }
    ...
    {
        return $this->getCredentials()['email'] ?? null;
    }
