...
<div class="bg-gray-50 min-h-screen">
    ...
    <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-12">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <!-- Lista de Vinis Cadastrados -->
            <div class="bg-white shadow overflow-hidden sm:rounded-lg mb-8 mt-10">
                <div class="px-4 py-5 sm:px-6">
                    <h3 class="text-lg leading-6 font-medium text-gray-900">
                        Discos Cadastrados
                    </h3>
                    <p class="mt-1 max-w-2xl text-sm text-gray-500">
                        Últimos lançamentos disponíveis em nossa loja
                    </p>
                </div>
                    ...
                    @if($vinyls->count() > 0)
                        <ul class="divide-y divide-gray-200">
                            @foreach($vinyls as $vinyl)
                                <li class="px-4 py-4 sm:px-6 hover:bg-gray-50 relative {{ $vinyl->is_available ? 'border-l-4 border-green-500' : 'border-l-4 border-red-300 bg-red-50 opacity-75' }}">
                                    <div class="flex items-start">
                                        <!-- Distintivo para produtos disponíveis -->
                                        ...
                                        <div class="flex-shrink-0 h-24 w-24 mr-4">
                                            @if($vinyl->cover_image)
                                                <img src="{{ asset('storage/' . $vinyl->cover_image) }}" alt="{{ $vinyl->title }}" class="h-24 w-24 object-cover rounded">
                                            @else
                                                <div class="h-24 w-24 bg-gray-200 rounded flex items-center justify-center text-gray-400">
                                                    <svg class="h-12 w-12" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zM10 2a8 8 0 00-8 8 7.92 7.92 0 003 6.243V13a1 1 0 011-1h8a1 1 0 011 1v3.243A7.92 7.92 0 0018 10 8 8 0 0010 2z" clip-rule="evenodd"></path></svg>
                                                </div>
                                            @endif
                                        </div>
                                        ...
                                        <div class="flex-1">
                                            <div class="flex items-center justify-between">
                                                <h4 class="text-lg font-medium text-gray-900">{{ $vinyl->title }}</h4>
                                                <p class="text-sm font-medium {{ $vinyl->is_available ? 'text-green-600' : 'text-gray-500' }}">
                                                    @if($vinyl->vinylSec && isset($vinyl->vinylSec->price))
                                            ...
                                            <div class="mt-1">
                                                ...
                                                @if($vinyl->vinylSec && $vinyl->vinylSec->format)
                                                <p class="text-sm text-gray-600">
                                                    <span class="font-medium">Formato:</span> 
                                                    {{ $vinyl->vinylSec->format }}
                                                </p>
                                                @endif
                                                
                                                <!-- Status da mídia e capa -->
                                                <div class="mt-2 flex flex-wrap gap-2">
                                                    @if($vinyl->vinylSec && $vinyl->vinylSec->midiaStatus)
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                                            Mídia: {{ $vinyl->vinylSec->midiaStatus->name }}
                                                        </span>
                                                    @endif
                                                    
                                                    @if($vinyl->vinylSec && $vinyl->vinylSec->coverStatus)
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                                            Capa: {{ $vinyl->vinylSec->coverStatus->name }}
                                                        </span>
                                                    @endif
                                                    ...
                                                    @if($vinyl->recordLabel)
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                                            @if($vinyl->recordLabel->logo)
                                                                <img src="{{ asset('storage/' . $vinyl->recordLabel->logo) }}" alt="{{ $vinyl->recordLabel->name }}" class="h-4 w-auto mr-1">
                                                            @endif
                                                            {{ $vinyl->recordLabel->name }}
                                                        </span>
                                                    @endif
                                                    
                                                    @if($vinyl->vinylSec && $vinyl->vinylSec->is_new)
                                                        <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                                            Novo
                                                        </span>
                                                    @else
                                                ...
                                                <!-- Faixas com YouTube links -->
                                                @if($vinyl->tracks->whereNotNull('youtube_url')->count() > 0)
                                                <div class="mt-3">
                                                    <p class="text-sm font-medium text-gray-700 mb-1">Faixas com vídeos:</p>
                                                    <ul class="text-xs text-gray-500 space-y-1">
                                                        @foreach($vinyl->tracks->whereNotNull('youtube_url')->take(3) as $track)
                                                            <li>
                                                                <a href="{{ $track->youtube_url }}" target="_blank" class="text-indigo-600 hover:text-indigo-900">
                                                                    {{ $track->position }}. {{ $track->name }} <svg class="inline-block h-3 w-3" fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg"><path d="M11 3a1 1 0 100 2h2.586l-6.293 6.293a1 1 0 101.414 1.414L15 6.414V9a1 1 0 102 0V4a1 1 0 00-1-1h-5z"></path><path d="M5 5a2 2 0 00-2 2v8a2 2 0 002 2h8a2 2 0 002-2v-3a1 1 0 10-2 0v3H5V7h3a1 1 0 000-2H5z"></path></svg>
                                                                </a>
                                                            </li>
                                                        @endforeach
                                                    </ul>
