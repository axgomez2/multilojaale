...
<nav class="bg-black border-b border-slate-800 w-full">
    <div class="max-w-7xl mx-auto flex items-center justify-between py-3 px-4 md:py-3 md:px-4">
        ...
        <div class="hidden md:flex items-center space-x-6 min-w-max">
            ...
            <!-- Favoritos -->
            @auth
            <!-- Usuário logado: Link para wishlist -->
            ...
            <div x-data="{ open: false }" class="relative" @keydown.escape.window="open = false">
                <a href="{{ route('site.cart.index') }}" class="flex flex-col items-center text-white hover:text-yellow-400 transition-colors relative focus:outline-none" title="Carrinho">
                    ...
                    @auth
                        ...
                        @endphp
                        @if($cartCount > 0)
                            <span class="cart-count absolute -top-2 -right-2 bg-yellow-400 text-slate-900 text-xs rounded-full px-1.5 py-0.5">{{ $cartCount }}</span>
                        @else
                            <span class="cart-count absolute -top-2 -right-2 bg-yellow-400 text-slate-900 text-xs rounded-full px-1.5 py-0.5 hidden">0</span>
                        @endif
                    @endauth
                </a>
